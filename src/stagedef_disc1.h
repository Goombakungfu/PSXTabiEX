	{ //StageId_1_1 (Bopeebo)
		//Characters
		{Char_BF_New,    FIXED_DEC(190,1),  FIXED_DEC(80,1)},
		{Char_Dad_New, FIXED_DEC(-60,1),  FIXED_DEC(80,1)},
		{Char_GF_New,     FIXED_DEC(110,1),    FIXED_DEC(-20,1)},
		
		//Stage background
		Back_Week2_New,
		
		//Song info
		{FIXED_DEC(18,10),FIXED_DEC(2,1),FIXED_DEC(23,10)},
		1, 1,
		XA_Bopeebo, 0,
		
		StageId_1_2, STAGE_LOAD_FLAG
	},
	{ //StageId_1_2 (Fresh)
		//Characters
		{Char_BF_New,    FIXED_DEC(190,1),  FIXED_DEC(80,1)},
		{Char_Dad_New, FIXED_DEC(-60,1),  FIXED_DEC(80,1)},
		{Char_GF_New,     FIXED_DEC(110,1),    FIXED_DEC(-20,1)},
		
		//Stage background
		Back_Week2_New,
		
		//Song info
		{FIXED_DEC(12,10),FIXED_DEC(14,10),FIXED_DEC(25,10)},
		1, 2,
		XA_Fresh, 2,
		
		StageId_1_3, STAGE_LOAD_STAGE | STAGE_LOAD_FLAG | STAGE_LOAD_PLAYER | STAGE_LOAD_GIRLFRIEND | STAGE_LOAD_OPPONENT
	},
	{ //StageId_1_3 (Dadbattle)
		//Characters
		{Char_bfs_New,    FIXED_DEC(220,1),  FIXED_DEC(80,1)},
		{Char_tabim_New, FIXED_DEC(-40,1),  FIXED_DEC(80,1)},
		{Char_gfs_New,     FIXED_DEC(130,1),    FIXED_DEC(70,1)},
		
		//Stage background
		Back_Week4_New,
		
		//Song info
		{FIXED_DEC(17,10),FIXED_DEC(21,10),FIXED_DEC(29,10)},
		1, 3,
		XA_Dadbattle, 0,
		
		StageId_1_3, 0
	},